...
function MediaList({title, medias}) {
    return(
                ...
                {medias.map((currentMedia)=>(
                    <li className='flex items-center mt-3' key={currentMedia.id}>
                        <label className='flex-1 flex'>
                            <input type="checkbox" checked={currentMedia.checked}  className='tasks-section__item__checkbox mt-1 mr-1.5 w-4 h-4 border-2 relative cursor-pointer '/>
                            <span className='tasks-section__item__text flex-1 cursor-pointer justify-center'>
                                {currentMedia.title}
                            </span>
                    </label>
                    <button type="button" className='flex min-w-3 min-h-3 text-green-800 mr-2 hover:bg-green-300 rounded-full'>
                    { IconEdit }                       
                    </button>
                    <button type="button" className='flex min-w-3 min-h-3 text-red-800 mr-4 hover:bg-red-300 rounded-full'>
                    { IconDelete }
                    </button>                    
                </li>
                ))
